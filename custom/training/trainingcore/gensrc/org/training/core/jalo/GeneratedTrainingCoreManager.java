...
import org.training.core.jalo.NityaProduct;
...
public abstract class GeneratedTrainingCoreManager extends Extension
{
	...
	public NityaProduct createNityaProduct(final SessionContext ctx, final Map attributeValues)
	{
		try
		{
			ComposedType type = getTenant().getJaloConnection().getTypeManager().getComposedType( TrainingCoreConstants.TC.NITYAPRODUCT );
			return (NityaProduct)type.newInstance( ctx, attributeValues );
		}
		catch( JaloGenericCreationException e)
		{
			final Throwable cause = e.getCause();
		...
		{
			throw new JaloSystemException( e ,"error creating nityaProduct : "+e.getMessage(), 0 );
		}
	}
	
	public NityaProduct createNityaProduct(final Map attributeValues)
	{
		return createNityaProduct( getSession().getSessionContext(), attributeValues );
	}
